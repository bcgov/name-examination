...
let givenQueue = (given, data) => {
  given(/^the name request queue contains:$/, (table) => {
    ...
    data.stubApi = function(options) {
      ...
      data.apiSandbox.getStub.withArgs('/api/v1/exact-match?query=' + encodeURIComponent(data.name(data.queueIndex)),
        sinon.match.any).returns(
        new Promise((resolve) => {
          resolve({
            data: {
              names: [
      ...
      data.apiSandbox.getStub.withArgs(
        '/api/v1/requests/synonymbucket/' + data.name(data.queueIndex).replace('&', ' ') + '/*', sinon.match.any)
        .returns(
          new Promise((resolve) => {
            resolve({
              data: {
      ...
      data.apiSandbox.getStub.withArgs(
        '/api/v1/requests/cobrsphonetics/' + data.name(data.queueIndex).replace('&', ' ') + '/*', sinon.match.any)
        .returns(
          new Promise((resolve) => {
            resolve({
              data: {
      ...
      data.apiSandbox.getStub.withArgs(
        '/api/v1/requests/phonetics/' + data.name(data.queueIndex).replace('&', ' ') + '/*', sinon.match.any).returns(
        new Promise((resolve) => {
          resolve({
            data: {
              names: []
